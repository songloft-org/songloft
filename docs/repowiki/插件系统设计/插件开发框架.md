...
- [plugins/mimusic-plugin-cloudflared/static/js/app.js](file://plugins/mimusic-plugin-cloudflared/static/js/app.js)
</cite>
...
## 简介
本文件面向插件开发者，系统性阐述 MiMusic JavaScript 插件开发框架的接口定义、路由注册机制、异步插件开发、健康监控、插件间通信、热更新等功能。框架基于 QuickJS JavaScript 引擎，采用进程内运行时管理，提供稳定的生命周期管理、异步回调处理、静态资源服务以及丰富的插件间通信能力。
...
- 官方插件示例：jsplugins-src/mimusic-jsplugin-*
- 插件发布配置：jsplugins/*
...
### 插件开发示例与最佳实践
- **开发步骤要点**：
  - 使用@mimusic/plugin-sdk创建路由和处理函数。
...
- **数据访问**：
  ...
  - 支持存储、歌曲、播放列表等数据操作。
- **插件间通信**：
  - 使用mimusic.comm.send()异步发送消息。
  - 使用mimusic.comm.call()同步调用其他插件。
  - 支持超时控制和错误处理。
- **热更新**：
  - 插件文件变化会自动触发热更新。
  - 支持无感更新，不影响其他插件运行。
...
**章节来源**
- [jsplugins-src/mimusic-jsplugin-lxmusic/src/main.ts:52-132](file://jsplugins-src/mimusic-jsplugin-lxmusic/src/main.ts#L52-L132)
- [jsplugins-src/mimusic-jsplugin-xiaomi/src/main.ts:40-132](file://jsplugins-src/mimusic-jsplugin-xiaomi/src/main.ts#L40-L132)
- [internal/jsplugin/communication.go:91-142](file://internal/jsplugin/communication.go#L91-L142)

### 官方插件示例
**新增** MiMusic提供了两个官方JavaScript插件示例，展示不同类型的插件开发模式：
...
**章节来源**
- [jsplugins-src/mimusic-jsplugin-lxmusic/src/main.ts:1-132](file://jsplugins-src/mimusic-jsplugin-lxmusic/src/main.ts#L1-L132)
- [jsplugins-src/mimusic-jsplugin-xiaomi/src/main.ts:1-132](file://jsplugins-src/mimusic-jsplugin-xiaomi/src/main.ts#L1-L132)
...
## 结论
MiMusic JavaScript插件开发框架通过现代化的架构设计，为插件开发者提供了强大而灵活的开发环境。基于QuickJS引擎的JavaScript运行时，支持真正的异步处理和Promise链，配合完善的健康监控、插件间通信和热更新机制，开发者可以构建高性能、高可用的插件应用。相比传统的WASM插件系统，JavaScript插件系统提供了更丰富的生态系统和更好的开发体验。
...
- **洛雪音乐插件示例**：jsplugins-src/mimusic-jsplugin-lxmusic/src/main.ts
- **小米音箱插件示例**：jsplugins-src/mimusic-jsplugin-xiaomi/src/main.ts
- **插件发布配置**：jsplugins/lxmusic.json, jsplugins/xiaomi.json
- **官方插件示例模板**：mimusic-jsplugin-lxmusic, mimusic-jsplugin-xiaomi
...
- **前端平台信息查询**：plugins/mimusic-plugin-cloudflared/static/js/common.js
- **平台映射配置**：plugins/mimusic-plugin-cloudflared/static/js/app.js