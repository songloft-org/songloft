...
## 简介
本指南面向首次接触 MiMusic 的用户，帮助你在最短时间内完成环境准备、安装与运行，并掌握基础认证流程（登录获取 Token、使用 Access Token 访问受保护接口、刷新 Token、登出）。同时提供 Docker 部署、常见启动问题排查与故障排除方法，确保你能顺利体验核心功能。

## 项目结构
MiMusic 采用 Go 语言开发，后端提供 RESTful API，内置 SQLite 存储，支持 JWT 双 Token 认证，提供 Swagger 文档与 Docker 镜像。前端可独立部署或嵌入后端，亦可配合 Flutter 客户端使用。
...
- Docker 部署
  ...
  - 运行容器：make docker-run（或 docker run -p 58091:58091 hanxi/mimusic:latest）
...
- 快速命令清单
  ...
  - ./mimusic -username admin -password admin -port 58091
- Swagger 文档
