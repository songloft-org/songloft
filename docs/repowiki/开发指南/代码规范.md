...
## 简介
本指南面向 MiMusic 项目，提供统一的代码规范与最佳实践，覆盖 Go 后端、Dart/Flutter 前端以及 Web/Vue 前端三部分。内容包括命名约定、代码格式、注释规范、错误处理模式、包组织结构、代码审查清单与自动化工具配置方法，帮助团队在多语言协作下保持一致的代码质量。
...
- 插件生态：mimusic-plugin 与 plugins 子项目提供插件协议与示例实现。
...
- 命名约定
  ...
  - 类名：帕斯卡命名，如 MiMusicApp、MiMusicAudioHandler。
...
participant Handler as "MiMusicAudioHandler"
Main->>Main : "全局异常捕获初始化"
