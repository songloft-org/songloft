...
```mermaid
graph TB
Docker["Dockerfile<br/>full/lite 构建参数"] --> Build["构建产物 mimusic"]
...
## 结论
MiMusic 项目通过前后端分离与同域直连，结合 go:embed 的静态资源嵌入策略，实现了简洁高效的交付方式。后端采用清晰的分层架构与中间件体系，具备良好的可维护性与可扩展性。插件系统以 WASM 为核心，提供了强大的扩展能力与与核心系统的深度集成。整体架构在性能、安全与可运维方面均体现了成熟的设计与工程实践。
