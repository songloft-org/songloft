...
把歌曲元数据完整回写到音频文件（`pkg/tag.WriteTag`，按扩展名 dispatch + 临时文件原子写入）。返回 `FileWriteStatus`（written/unchanged/skipped/failed）而非 error，实现"DB 必须成功，文件失败可降级"的半成功语义。写入前通过 `tagsUnchanged` 对比现有标签，一致则跳过。支持写入的格式：MP3、FLAC、APE、WAV、M4A/MP4/M4B/MOV、OGG/OGA、AIFF/AIF（文本、歌词、封面均可写，唯 WAV 受格式限制不支持封面）；其余扩展名返回 `ErrUnsupportedWrite`，由调用方降级为日志、不阻塞主流程。
