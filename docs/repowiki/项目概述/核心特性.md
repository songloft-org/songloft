...
## 简介
MiMusic 是一个基于 Go 与 Chi 路由框架的轻量级音乐服务器，提供本地音乐文件扫描、元数据提取、歌单管理、JWT 双 Token 认证、RESTful API、Docker 支持、SQLite 数据库、灵活配置、WASM 插件系统与令牌管理等能力。其整体采用前后端分离架构，后端通过嵌入式前端资源提供同域访问，结合 WebAssembly 插件实现高度可扩展的功能扩展。
...
- 外部依赖
  ...
  - 插件 HTTP 库：github.com/mimusic-org/plugin/pkg/go-plugin-http
- 内部模块
...
## 结论
MiMusic 通过清晰的分层架构与插件化设计，实现了从音乐管理到认证与扩展的完整能力闭环。WASM 插件系统显著提升了可扩展性，SQLite 与 Hanxi/Tag 的组合兼顾了易用性与性能。结合 Docker 支持与灵活配置，MiMusic 能够满足个人与小型团队的本地音乐服务需求，并为后续功能扩展提供稳定基础。
